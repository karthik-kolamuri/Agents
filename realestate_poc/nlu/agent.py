...
nlu_intent_agent = Agent(
    ...
    # Completely removed `db=agent_db` and `add_history_to_context=True` to eliminate the 5s database latency on every inference! Context is handled manually in workflow.py now.
